...
public class RobotContainer {
  ...
  */
  ...
  private void configureButtonBindings() {
    robotController.leftBumper().whileTrue(Commands.run(
      m_intakeSystem::runIntakeWheels, m_intakeSystem));

    robotController.leftBumper().onFalse(Commands.runOnce(
      m_intakeSystem::stopIntakeWheels, m_intakeSystem));

    robotController.rightBumper().whileTrue(Commands.run(
      m_dumperSystem::raiseDumper, m_dumperSystem));

    robotController.rightBumper().onFalse(Commands.run(
      m_dumperSystem::lowerDumper, m_dumperSystem));

    robotController.a().onTrue(Commands.runOnce(
      m_intakeSystem::changeIntakeDirection, m_intakeSystem));

    robotController.povLeft().onTrue(Commands.runOnce(
      m_driveSystem::lowerMaxOutput, m_driveSystem));

    robotController.povRight().onTrue(Commands.runOnce(
      m_driveSystem::raiseMaxOutput, m_driveSystem));
  }
